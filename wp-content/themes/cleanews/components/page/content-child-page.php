...
<li class="clear archive-item list-child-pages group">
    <?php
        if ( cleanews_post_icon() ) { ?>
            <a class="list-link" href="<?php echo esc_url( get_permalink() ); ?>" title="<?php 
                    echo esc_attr__( 'See more info about ', 'cleanews' ) . get_the_title(); ?>">
    ...
    <div class="<?php echo cleanews_post_icon() ? 'medium-9' : 'medium-12'; ?> columns">
    <a class="list-link" href="<?php echo esc_url( get_permalink() ); ?>" title="<?php
                    echo esc_attr__( 'See more info about ', 'cleanews' ) . get_the_title(); ?>">
        <h2 class="entry-list-title <?php echo ! cleanews_post_icon() ? 'text-center' : ''; ?>"><?php echo get_the_title(); ?></h2>
    </a>
    <?php cleanews_fancy_excerpt(); ?>
    </div>
