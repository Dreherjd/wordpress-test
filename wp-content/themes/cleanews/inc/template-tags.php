...
if ( ! function_exists( 'cleanews_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
function cleanews_posted_on() {
        global $post;
    ...
    $byline = sprintf(
        ...
        esc_html_x( 'By %s', 'post author', 'cleanews' ),
    ...
    //echo '<span class="byline"> ' . $byline . '</span>';
        ...
        /* translators: used between list items, there is a space after the comma */
                if( 'post' === get_post_type() ) {
                    $categories_list = get_the_category_list( __( '</li><li>', 'cleanews' ) );
...
if ( ! function_exists( 'cleanews_entry_footer' ) ) :
/**
 * Prints HTML with meta information for the categories, tags and comments.
 */
function cleanews_entry_footer() {
    global $post;
    ...
    if ( 'post' === get_post_type() || 'jetpack-portfolio' === get_post_type() ) {
        /* translators: used between list items, there is a space after the comma */
            if( 'post' === get_post_type() ) {
        $categories_list = get_the_category_list( esc_html__( ', ', 'cleanews' ) );
            } elseif( 'jetpack-portfolio' === get_post_type() ) {
                $categories_list = get_the_term_list( $post->ID, 'jetpack-portfolio-type', '', esc_html_x(', ', 'Used between list items, there is a space after the comma.', 'cleanews' ), '');
            }
        if ( $categories_list && cleanews_categorized_blog() ) {
    ...
    if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
        ...
        comments_popup_link( esc_html__( 'Leave a comment', 'cleanews' ), esc_html__( '1 Comment', 'cleanews' ), esc_html__( '% Comments', 'cleanews' ) );
        echo '</span>';
    }
}
endif;
 ...
 * @return bool
 */
function cleanews_categorized_blog() {
    if ( false === ( $all_the_cool_cats = get_transient( 'cleanews_categories' ) ) ) {
        ...
        set_transient( 'cleanews_categories', $all_the_cool_cats );
    }

    if ( $all_the_cool_cats > 1 ) {
        // This blog has more than 1 category so cleanews_categorized_blog should return true.
        return true;
    } else {
        // This blog has only 1 category so cleanews_categorized_blog should return false.
        return false;
    }
}

/**
 * Flush out the transients used in cleanews_categorized_blog.
 */
function cleanews_category_transient_flusher() {
    if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
        return;
    }
    // Like, beat it. Dig?
    delete_transient( 'cleanews_categories' );
}
add_action( 'edit_category', 'cleanews_category_transient_flusher' );
add_action( 'save_post',     'cleanews_category_transient_flusher' );



/*==============================================================================
 * cleanews CUSTOM TAGS BELOW
 =============================================================================*/
/**
 * Fancy excerpts
 * 
 ...
 */
function cleanews_fancy_excerpt() {
    global $post;
    if( is_archive() ) {
       the_excerpt();
        echo '<div class="continue-reading">';
        echo '<a class="more-link more-link-activated" href="' . esc_url( get_permalink() ) . '" title="' . esc_html__( 'Continue Reading ', 'cleanews' ) . get_the_title() . '" rel="bookmark">' . esc_html__( 'Continue Reading', 'cleanews' ) . '</a>'; 
        echo '</div>';
    } elseif ( is_page_template( 'page-templates/page-child-pages.php' ) ) {
        the_excerpt();
         echo '<div class="continue-reading">';
        echo '<a class="continue-reading-arrow" href="' . esc_url( get_permalink() ) . '" title="' . esc_html__( 'Continue Reading ', 'cleanews' ) . get_the_title() . '" rel="bookmark">&rarr;</a>'; 
        echo '</div>';
...
function cleanews_author_box() {
    ...
    if ( isset( $post->post_author ) ) {
        ...
        $author_details .= '<h3 class="author-title"><span>' . esc_html__( 'About ', 'cleanews' );
            if ( is_author() ) $author_details .= $display_name;        // If an author archive, just show the author name
            else $author_details .= esc_html__( 'the Author', 'cleanews' ); // If a regular page, show "About the Author"
        ...
        if ( ! is_author() ) {  // Don't show the meta info on an author archive page
            $author_details .= '<p class="author-links entry-meta"><span class="vcard">' . esc_html__( 'All posts by', 'cleanews' ) . '<a class="fn" href="' . esc_url( $user_posts ) . '">' . $display_name . '</a></span>';

            // Check if author has a website in their profile
            if ( ! empty( $user_site ) ) 
                $author_details .= '<a class="author-site" href="' . esc_url( $user_site ) . '" target="_blank" rel="nofollow">' . esc_html__( 'Website', 'cleanews' ) . '</a></p>';
            else $author_details .= '</p>';
        }
        
        $author_details .= '</section>';
        $author_details .= '</div>';
        $author_details .= '<p class="show-hide-author label">' . esc_html__( 'Hide', 'cleanews' ) . '</p>';
...
function cleanews_portfolio_index_footer() {
    $time_string = '<time class="entry-date published" datetime="%1$s">%2$s</time>';
    if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
            $time_string = '<i class="fa fa-refresh tip"><span class="tooltip">' .
                        esc_attr__( 'First posted: ', 'cleanews' ) .
    ...
    $project_type = get_the_term_list( get_the_ID(), 'jetpack-portfolio-type', '<span class="portfolio-entry-meta cat-links">', esc_html_x( ', ', 'Used between list items, there is a space after the comma.', 'cleanews' ), '</span>' );
...
if ( ! function_exists( 'cleanews_breadcrumbs' ) ) :
/**
...
function cleanews_breadcrumbs() {
    ...
    $home_link = '<a aria-label="' . esc_html__( 'Home', 'cleanews' ) .'" title="' . esc_html__( 'Home', 'cleanews' ) .'" class="breadcrumb-home" href="' . esc_url( home_url() ) . '"><i class="fa fa-home"></i></a>' . $separator;
    
    $output .= "<div aria-label='" . esc_html__( 'You are here:', 'cleanews' ) . "' id='$breadcrumb_id' class='$breadcrumb_class'>";
    $output .= $home_link;
...
function cleanews_the_post_icon() {
    echo wp_kses( cleanews_post_icon(), array( 'span' => array( 'class' => array() ) ) );
}

/**
 * Function to show the Jetpack sharing and Likes only at the designated locations in Posts and Pages
 */
function cleanews_jetpack_sharing() {
 ...
 */
function cleanews_post_navigation() {
    ...
    <nav class="navigation post-navigation" role="navigation">
        <p class="screen-reader-text"><?php esc_html_e( 'Post navigation', 'cleanews' ); ?></p>
        <div class="nav-links" data-equalizer>
                <?php
                        previous_post_link( '<div class="nav-previous" data-equalizer-watch><div class="nav-indicator">' . esc_html_x( 'Previous Post:', 'Previous post', 'cleanews' ) . '</div><h4>%link</h4></div>', '%title' );
                        next_post_link(     '<div class="nav-next" data-equalizer-watch><div class="nav-indicator">'     . esc_html_x( 'Next Post:', 'Next post', 'cleanews' ) . '</div><h4>%link</h4></div>', '%title' );
                ?>
        </div> <!-- .nav-links -->
    </nav> <!-- .navigation -->
    <?php
...
if ( ! function_exists( 'cleanews_paging_nav' ) ) :
/**
 ...
 */
function cleanews_paging_nav() {
        the_posts_pagination( array(               
            'mid_size'      => 3,
            'prev_text' => __( 'Previous', 'cleanews' ),
            'next_text' => __( 'Next', 'cleanews' ),
            'type'      => 'list',
        ));
}
endif;
...
if ( ! function_exists( 'cleanews_copyright' ) ) :
/** 
