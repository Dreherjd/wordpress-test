 ...
 */
function cleanews_customize_register( $wp_customize ) {
	$wp_customize->get_setting( 'blogname' )->transport         = 'postMessage';
	$wp_customize->get_setting( 'blogdescription' )->transport  = 'postMessage';
    $wp_customize->get_section('background_image')->title = __( 'Background', 'cleanews' );

    $wp_customize->add_section(
        'navigation_settings',
        array(
            'title' => __('Navigation', 'cleanews'),
            'priority' => 99,
            )
        );  
    ...
    $wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'nav_bg', array(
        'label'       => __( 'Background Color', 'cleanews' ),
    ...
    $wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'nav_link_color', array(
        'label'       => __( 'Link Color', 'cleanews' ),
        'section'     => 'navigation_settings',
        'priority' => 1,
        'settings'    => 'nav_link_color',
        ) ) );
    $wp_customize->add_control( 'display_header_text', array(
        'label'    => __( 'Header Text Color', 'cleanews' ),
        'section'  => 'head_colors',
        ) );
    $wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'background_color', array(
        'label'       => __( 'Background Color', 'cleanews' ),
        'description' => __( 'Choose a color for the background.', 'cleanews' ),
        'section'     => 'background_image',
        'settings'    => 'background_color',
        ) ) );
    $wp_customize->add_setting( 'show_sitename_in_menubar', array(
        'default'           => 1,
        'sanitize_callback' => 'cleanews_sanitize_checkbox',
        ) );
    $wp_customize->add_section(
        'scroll_to_top_section',
        array(
            'title' => __('Scroll To Top Button ', 'cleanews'),
            'priority' => 99,
            )
        );  
    $wp_customize->add_setting( 'scroll_top_background', array(
        'default'           => '#f19772', 
        'type'              => 'theme_mod',
        'sanitize_callback' => 'sanitize_hex_color',
        'transport'         => 'postMessage'
    ...
    $wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'scroll_top_background', array(
        'label'       => __( 'Scroll To Top Background Color', 'cleanews' ),
        'section'     => 'scroll_to_top_section',
        'priority' => 1,
        'settings'    => 'scroll_top_background',
        ) ) );
    $wp_customize->add_setting( 'scroll_top_color', array(
        'default'           => '#fff', 
        'type'              => 'theme_mod',
        'sanitize_callback' => 'sanitize_hex_color',
        'transport'         => 'postMessage'
    ...
    $wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'scroll_top_color', array(
        'label'       => __( 'Scroll To Top Icon Color', 'cleanews' ),
        'section'     => 'scroll_to_top_section',
        'priority' => 1,
        'settings'    => 'scroll_top_color',
        ) ) );
    ...
    $wp_customize->add_control(
        new WP_Customize_Control(
            ...
            array( 
                'label'         => __( 'Show sitename in menu bar?', 'cleanews' ),
                'type'          => 'checkbox',
    ...
    /* ///////////////// SIDEBAR LAYOUT ////////////////// */
        ...
        $wp_customize->add_section( 'cleanews-options', array(
            'title'         => __( 'Theme Options', 'cleanews' ),
            'capability'    => 'edit_theme_options',
            'description'   => __( 'Change the default display options for the theme.', 'cleanews' ),
            ) );
        ...
        $wp_customize->add_setting( 'layout_setting',
            array(
                ...
                'type'              => 'theme_mod',
                'sanitize_callback' => 'cleanews_sanitize_layout',
                'transport'         => 'postMessage'
                ) );
        ...
        $wp_customize->add_control( 'layout_control',
            array(
                ...
                'type'              => 'radio',
                'label'             => __( 'Sidebar position', 'cleanews' ),
                'choices'           => array(
                    'no-sidebar'    => __( 'No sidebar', 'cleanews' ),
                    'sidebar-right' => __( 'Sidebar right', 'cleanews' ),
                    'sidebar-left'  => __( 'Sidebar left', 'cleanews' ),
                    ),
...
//			'transport'         => 'postMessage',
          ...
          $wp_customize->add_control( 'panel_' . $page_names[$i], array(
           ...
           'label'          => sprintf( __( '%s Page Content', 'cleanews' ), ucwords( $page_names[$i] ) ),
           'description'    => ( 0 !== $i ? '' : __( 'Select pages to feature in each area from the dropdowns. Add an image to a section by setting a featured image in the page editor. Empty sections will not be displayed.', 'cleanews' ) ),
           'section'        => 'static_front_page',
           'type'           => 'dropdown-pages',
           'allow_addition' => true,
           'active_callback' => 'cleanews_is_static_front_page',
           ) );

//		$wp_customize->selective_refresh->add_partial( 'panel_' . $page_names[$i], array(
//			'selector'            => '#' . $page_names[$i],
//			'render_callback'     => 'cleanews_front_page_section',
//			'container_inclusive' => true,
//		) );
      }

  }
  add_action( 'customize_register', 'cleanews_customize_register' );

/**
 * Return whether we're previewing the front page and it's a static page.
 */
function cleanews_is_static_front_page() {
 ...
 */
function cleanews_customize_preview_js() {
	wp_enqueue_script( 'cleanews_customizer', get_template_directory_uri() . '/assets/js/customizer.js', array( 'customize-preview' ), '20160507', true );
}
add_action( 'customize_preview_init', 'cleanews_customize_preview_js' );

/*
 * Sanitize layout options
 */

function cleanews_sanitize_layout ( $value ) {
 ...
 */
function cleanews_sanitize_checkbox( $checked ) {
    // Boolean check
    return ( ( isset( $checked ) && true == $checked ) ? true : false );
}


if(! function_exists('cleanews_get_user_input' ) ):
    function cleanews_get_user_input(){
        ?>
        <style type="text/css">
        ...
        <?php }
        add_action( 'wp_head', 'cleanews_get_user_input' );
        endif;
